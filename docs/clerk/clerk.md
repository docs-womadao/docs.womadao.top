...
4. **自动宵禁**：当自动宵禁功能开启，工作日00:00-06:00自动开启全员禁言
...
| [词云](#3-2-2) | - | ?[时间范围] ?[参数] | 所有人 | 根据聊天记录生成词云 | `@店员` 词云 -u 1周 | [点击查看具体用法](#3-2-2) |
...
- **用法**：@店员 词云 [时间范围] [选项]
- **时间范围**（默认为'1天'）：
    > 以下提到的**所有沃频子频道**不包括店员与用户的私信
    > 词云会自动排除机器人的发言（包括店员和其他已被标记的机器人）
    - **可用单位**：秒/s、分钟/m、小时/h、天/d、周/w、月/M、年/y
    - **格式**：**数字**+**单位**，例如'1天'、'1周'、'1小时'
- **选项**：
    -  **当前子频道模式**（'**-c**'）：获取**当前子频道**的**所有用户**聊天记录生成的词云 
    -  **用户模式**（'**-u**'）：获取**当前使用指令的用户**在**所有沃频子频道**中的聊天记录生成的词云 
    -  **全子频道模式**（'**-g**'）：获取**所有沃频子频道**中**所有用户**的聊天记录生成的词云
    - 注：使用**用户模式**时，词云**默认私信**发送给用户，您可以附加'**-p**'参数来指定发送到当前子频道
- **示例**：
    - 查看自己1天内在**所有沃频子频道**中的发言的词云：“@店员 词云 1天 -u”
    - 查看本子频道（本群）一周内所有用户的发言的词云：“@店员 词云 1周 -c”
    - 查看**所有沃频子频道**一年内所有用户的发言的词云：“@店员 词云 1年 -g”
