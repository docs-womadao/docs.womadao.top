...
| 排行榜 | 略 | 所有人 | 查询沃玛岛服务器统计信息排行，具体用法请使用 “`@店员`  排行榜” 来获取 | `@店员`  排行榜-<?页数> <统计分类> <?统计项> |
| 分类排行榜 | 略 | 所有人 | 查询沃玛岛服务器统计信息的某个分类下的所有条目的统计排行，具体用法请使用“`@店员`  排行榜” 来获取 | `@店员`  分类排行榜-<?页数> <统计分类> <?玩家名> <升序/降序> |
| 排名 | 略 | 所有人 | 查询指定玩家名在沃玛岛服务器统计信息中某个条目的排行名次，具体用法请使用“`@店员`  排名”来获取 | `@店员`  排名 <玩家名> <统计分类> <?统计项> |
| 记忆清除 | - | 群主/群管理员/管理员 | 清除店员AI聊天在当前频道的短期记忆 | `@店员`  记忆清除 |
| 重启 | - | 管理员 | 重启店员主进程 | `@店员`  重启 |
| git | pull | 管理员 | 从仓库拉取最新代码并重启 | `@店员` git pull |
| 扫雷 | 开始/显示/结束/帮助 | 所有人 | 扫雷小游戏 | `@店员` 扫雷 开始 简单 | 具体命令请使用 "`@店员` 扫雷 帮助" 来查看
